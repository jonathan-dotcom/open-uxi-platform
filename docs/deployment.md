...
## 9. Publish the Web Dashboard

The repository ships a React/Vite dashboard under `dashboard/` that visualizes
pipeline snapshots. Deploy it alongside Grafana (for example behind the same
reverse proxy) so operators can review synthetic checks without logging into
Grafana.

### 9.1 Build Requirements

- Node.js 18.x or newer (matching the version you use for other Vite apps).
- npm 9.x+ (or `pnpm`/`yarn`, adjust commands accordingly).

### 9.2 Configure Environment Variables

Create a `.env.production` file under `dashboard/` (or export variables in your
CI/CD system) before building. The most common settings are:

```ini
# Base URL for the pipeline server REST API
VITE_DASHBOARD_API_BASE=https://monitor.example.com/pipeline

# Optional: explicit WebSocket endpoint if different from API base
VITE_DASHBOARD_STREAM_URL=wss://monitor.example.com/pipeline/stream

# Optional: override stream port when deriving from API base
# VITE_DASHBOARD_STREAM_PORT=8766
```

If you omit `VITE_DASHBOARD_API_BASE`, the dashboard falls back to the bundled
`/data/dashboard.json` snapshot and sample data (`src/data/sampleData.ts`).
...
The static assets land in `dashboard/dist/`. Copy that directory to your web
server (for example `/var/www/uxi-dashboard`) and serve it through Nginx or a
static hosting provider. Remember to configure TLS and HTTP caching headers as
appropriate for your environment.

### 9.4 Continuous Delivery Tips

- Add the build commands to your CI pipeline so each Git push produces a fresh
  artifact.
- Use an infrastructure-as-code tool (Ansible, Terraform, etc.) to synchronize
  the built assets onto the cloud host alongside the Docker services.
- When hosting on the same domain as Grafana, configure your reverse proxy to
  forward `/pipeline` (API) to the pipeline server container and `/dashboard`
  (static assets) to the published files.
